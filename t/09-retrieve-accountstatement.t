...
use_ok("Finance::Bank::Postbank::Account");

my @new_args;
my $postbank = Test::MockObject->new();
$postbank->fake_module("Finance::Bank::Postbank", new => sub { @new_args = @_; $postbank; } );
...
is_deeply(\@new_args, [ 'Finance::Bank::Postbank', login => '9999999999', password => '11111' ], "Check for number => login conversion");
my ($func,$args) = $postbank->next_call();
is_deeply([$func,$args], [ "get_account_statement",[$postbank]], "get_account_statement() was called");
$postbank->clear;

$account = Finance::Bank::Postbank::Account->parse_statement(
                number => '0999999999',
                login => '9999999999',
                password => '11111',
              );
is_deeply(\@new_args, [ 'Finance::Bank::Postbank', login => '9999999999', password => '11111' ], "Check for login parameter");
