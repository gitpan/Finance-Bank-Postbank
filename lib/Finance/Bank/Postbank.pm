...
use WWW::Mechanize;
use Finance::Bank::Postbank::Account;
...
BEGIN {
  Finance::Bank::Postbank->mk_accessors(qw( agent ));
};
...
sub get_account_statement {
  ...
  if ($agent->status == 200) {
    return Finance::Bank::Postbank::Account->parse_statement(content => $agent->content);
  } else {
...
Finance::Bank::Postbank - Check your Postbank bank account from Perl
...
is either undef or a Finance::Bank::Postbank::Account object.
